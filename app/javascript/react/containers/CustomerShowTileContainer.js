...
class CustomerShowTileContainer extends React.Component {
  ...
  componentDidMount() {
    fetch(`/api/v1/users/${this.props.match.params.user_id}/customers/${this.props.match.params.id}`).then((response) => {
      if (response.ok) {
        return response.json()
      } else {
        let errorMessage = `${response.status} (${response.statusText})`,
  ...
  render() {
    ...
    if (this.state.diary.length === 0) {
      ...
      entrypage = this.state.diary.map((entry) => {
        ...
        return (<DiaryEntriesComponent key={entry.diary.id} information={entry.diary} owner={entry.user} currentUser={this.state.current_user} handlePinClick={this.handlePinClick} handleDeleteClick={this.handleDeleteClick}/>)
      })
    }

    return (<div className="customer-show grid-x messages-container">
      <div className="cell small-3 callout-diary-creatio grid-y">
        <CustomerDashboard customerInfo={this.state.current_customer} currentUser={this.state.designated_user} handleReload={this.handleReload} selectedDiary="selectedDiary" ={this.state.selectedDiary}/>
      </div>
      <div className="cell auto grid-container ">
        <DiaryFormContainer formPayload={this.formPayload}/>
