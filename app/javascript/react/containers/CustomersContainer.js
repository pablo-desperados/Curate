...
class CustomersContainer extends React.Component {
  ...
  fetchUserCustomers() {
    fetch(`/api/v1/users/${this.props.match.params.id}/customers`).then((response) => {
      if (response.ok) {
        return response.json()
      } else {
        let errorMessage = `${response.status} (${response.statusText})`,
  ...
  render() {
    ...
    if (this.state.customerList.length === 0) {
      ...
      customers = <EmptyCustomerContainer userId={userId}/>
    } else {
      customers = this.state.customerList.reverse().map(customer => {
        ...
        return (<CustomerCardContainer key={customer.id} id={customer.id} handleCardClick={this.handleCardClick} full_date={full_date} first_name={customer.first_name} last_name={customer.last_name} company={customer.company_name} status={customer.lifecycle_status} profile_picture={customer.profile_picture}/>)
      })
    }
